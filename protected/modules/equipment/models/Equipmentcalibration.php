...
class Equipmentcalibration extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('usageID, user_id, calibrationdata, remarks', 'required'),
			array('usageID, user_id', 'numerical', 'integerOnly'=>true),
			array('remarks', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('ID, usageID, user_id, calibrationdata, remarks', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('usageID',$this->usageID);
		$criteria->compare('user_id',$this->user_id);
		$criteria->compare('calibrationdata',$this->calibrationdata,true);
